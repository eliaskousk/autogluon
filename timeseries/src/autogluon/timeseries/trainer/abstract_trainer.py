...
from ..utils.metric_utils import check_get_evaluation_metric, METRIC_COEFFICIENTS
...
class AbstractTimeSeriesTrainer(SimpleAbstractTrainer):
    ...
    def fit_ensemble(self, val_data, model_names):
        ...
        higher_is_better = METRIC_COEFFICIENTS[self.eval_metric] == 1  # Note: This is hacky, ideally should be accessible from metric
    ...
    def score(
        ...
        if isinstance(model, TimeSeriesEnsembleWrapper):
            ...
            model_score = evaluator(
                data, forecasts
            ) * METRIC_COEFFICIENTS[eval_metric]
            return model_score
